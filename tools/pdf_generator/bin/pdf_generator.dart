...
void main(List<String> arguments) async {
  ...
  // Buscamos service-account.json en la raíz.
  ...
  if (!await serviceAccountFile.exists()) {
    ...
    print('Instrucciones para generarlo:');
    print('1. Ve a Firebase Console -> Project Settings -> Service Accounts.');
    print('2. Haz clic en "Generate new private key".');
    print('3. Guarda el archivo como "service-account.json" en la raíz del proyecto.');
    return;
  }

  print('Inicializando Firebase Admin...');
  final admin = FirebaseAdmin.app(
    AppOptions(
      credential: ServiceAccountCredential.fromFile(serviceAccountFile),
    ),
  );

  final firestore = admin.firestore();
  final collection = 'admin_selection_2026_05';

  print('Obteniendo pizzas de la colección $collection...');
  final snapshot = await firestore.collection(collection).orderBy('createdAt').get();
  ...
  // Opcional: Cargar fuentes si es necesario, por ahora usaremos las estándar de PDF
  // para evitar problemas de path.
  ...
  for (final doc in docs) {
    ...
    // Descargar imágenes
    final imageUrls = Map<String, dynamic>.from(data['imageUrls'] ?? {});
    final List<Uint8List> imageBytesList = [];

    for (final entry in imageUrls.entries) {
      try {
        print('  Descargando imagen: ${entry.key}...');
...
pw.Widget _buildTechnicalDetails(Map<String, dynamic> data) {
  return pw.Column(
    ...
    children: [
      ...
      _buildDetailRow('Estilo', data['pizzaStyle'] ?? '-'),
      _buildDetailRow('Harinas', data['flours'] ?? '-'),
      _buildDetailRow('Prefermento', '${data['preferment'] ?? '-'} (${data['prefermentPercentage'] ?? '-'}%)'),
...
pw.Widget _buildIngredients(Map<String, dynamic> data) {
  return pw.Column(
    ...
    children: [
      ...
      pw.Text('Otros ingredientes:', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10, color: kSecondaryColor)),
    ...
    child: pw.RichText(
      text: pw.TextSpan(
        children: [
          pw.TextSpan(
            text: '$label: ',
            style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10, color: kSecondaryColor),
          ),
          pw.TextSpan(
            text: value,
